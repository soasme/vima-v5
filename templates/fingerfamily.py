    ...
    if page['type'] == 'object':
        ...
        make_finger_page(
            ...
            audiences=set([
                ...
                for audience in
                (config['clips'][0:10] if index < 10 else config['clips'][10:20])
...
def main():
    args = parse_args()
    with open(f"{config['input_dir']}/config.json") as f:
        pages = json.load(f)

    config = {}
    config['input_dir'] = args.input_dir

    os.environ['ASSET_PATH'] = config['input_dir'] + ',' + os.environ.get('ASSET_PATH', '')

    for index, page in enumerate(pages['clips']):
